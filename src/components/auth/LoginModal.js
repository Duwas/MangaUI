  ...
  return (
    ...
    >
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        <h2 style={{ color: '#fff', fontSize: 24, fontWeight: 700, margin: 0 }}>Đăng Nhập</h2>
        <p style={{ color: '#9CA3AF', fontSize: 14, marginTop: 8 }}>Chào mừng bạn trở lại ComicVerse</p>
      </div>
      ...
      <Divider style={{ borderColor: '#2D2D4A' }}><span style={{ color: '#6B7280', fontSize: 12 }}>hoặc</span></Divider>
      <div style={{ display: 'flex', gap: 12 }}>
        <Button icon={<GoogleOutlined />} block size="large" style={{ flex: 1 }}>Google</Button>
        <Button icon={<FacebookOutlined />} block size="large" style={{ flex: 1 }}>Facebook</Button>
      </div>
      <p style={{ textAlign: 'center', marginTop: 20, color: '#9CA3AF', fontSize: 14 }}>
        Chưa có tài khoản?{' '}
        <a onClick={onSwitchToRegister} style={{ color: '#1DB954', cursor: 'pointer', fontWeight: 600 }}>Đăng ký ngay</a>
      </p>
    </Modal>
  );
}
